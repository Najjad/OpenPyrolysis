...
# ----------------------------- Example: reactor geometry -----------------------------
if __name__ == '__main__':
    # This example builds a simple reactor: a large cylinder (reactor body) with an inner tube removed
    # and an inlet port subtracted. It then samples the SDF on a grid and writes a mesh if scikit-image
    # and trimesh are installed.

    # Parameters (tweak as needed)
    reactor_radius = 0.5
    reactor_length = 2.0
    inner_tube_radius = 0.1
    inner_tube_length = 1.8
    inlet_radius = 0.05

    # build geometry
    outer = Cylinder(radius=reactor_radius, height=reactor_length, center=(0,0,0))
    inner = Translate(Cylinder(radius=inner_tube_radius, height=inner_tube_length, center=(0,0,0)), offset=(0,0,0))
    reactor = Difference(outer, inner)

    # add an inlet hole on the side
    inlet = Translate(Cylinder(radius=inlet_radius, height=0.3, center=(0,0,0)), offset=(reactor_radius-0.01, 0, 0))
    reactor_with_inlet = reactor - Rotate(inlet, axis=(0,1,0), angle_rad=math.pi/2)

    # sample grid
    bounds = ((-0.6,0.6), (-0.6,0.6), (-1.1,1.1))
    res = (160, 160, 240)
    print("Evaluating SDF on a grid (this may take a while)...")
    pts, vals = evaluate_on_grid(reactor_with_inlet, bounds, res)
    print("Done sampling. sdf shape:", vals.shape)

    # Try marching cubes (if available)
    try:
        verts, faces = mesh_from_grid(vals, bounds, level=0.0)
        print("Extracted mesh: verts", verts.shape, "faces", faces.shape)
        try:
            export_stl(verts, faces, 'reactor.stl')
            print('Saved reactor.stl')
        except Exception as e:
            print('STL export failed (trimesh missing?):', e)
    except Exception as e:
        print('Mesh extraction failed (skimage missing?):', e)
